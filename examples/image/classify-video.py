...
def main(argv):
    ...
    with ImageImpulseRunner(modelfile) as runner:
        try:
            ...
            print(
                f'Loaded runner for "{model_info["project"]["owner"]} / {model_info["project"]["name"]}"'
            )
            ...
            while img.size != 0:
                ...
                if "object_tracking" in res["result"].keys():
                    print(
                        "Found %d tracked objects"
                        % (len(res["result"]["object_tracking"]))
                    )
                    for obj in res["result"]["object_tracking"]:
                        print(
                            "\tID=%s, label=%s, value=%.2f, x=%d, y=%d, w=%d, h=%d"
                            % (
                                obj["object_id"],
                                obj["label"],
                                obj["value"],
