...
def main(argv):
    ...
    with ImageImpulseRunner(modelfile) as runner:
        try:
            ...
            if "thresholds" not in model_info["model_parameters"]:
                print(
                    "This model does not expose any thresholds, build a new Linux deployment (.eim file) to get configurable thresholds"
                )
                exit(1)
            ...
            while True:
                try:
                    ...
                    if threshold_id not in [
                        t["id"] for t in model_info["model_parameters"]["thresholds"]
                    ]:
